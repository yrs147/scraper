...
func Scrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
	Pages, err := buildUrls(searchTerm, countryCode, languageCode, pages, count)
	if err != nil {
		return nil, err
	}
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("Scraper recievees a non -200 status code suggesting a ban")
		return nil, err
	}
...
func main() {
	res, err := Scrape("Yash Raj", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
